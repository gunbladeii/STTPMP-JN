...
function simpanSyorBaru() {
  ...
  document.getElementById("catatanBaru").value =
    quillCatatanBaru.root.innerHTML;
...
function simpanSyorBaru2() {
  ...
  document.getElementById("catatanBaru2").value =
    quillCatatanBaru2.root.innerHTML;
  ...
  google.script.run
    .withSuccessHandler(() => {
      bootstrap.Modal.getInstance(
        document.getElementById("tambahModal2")
      ).hide();
      loadDataTab3();
      btn.disabled = false;
      btn.innerHTML = originalText;
    ...
    .withSuccessHandler(function (data) {
      ...
      if (!data || data.length === 0) {
        document.getElementById(
          "dataBody1"
        ).innerHTML = `<tr><td colspan='11' class='text-center text-danger fw-bold'>Akses tidak dibenarkan</td></tr>`;
        return;
      }
  ...
  google.script.run
    .withSuccessHandler(function (data) {
      if (!data || data.length === 0) {
        document.getElementById(
          "dataBody2"
        ).innerHTML = `<tr><td colspan='9' class='text-center text-danger fw-bold'>Akses tidak dibenarkan</td></tr>`;
        return;
      }
...
function renderTableTab3(data) {
  ...
  data.forEach((item, idx) => {
    ...
    row.innerHTML = `
          ...
          <td><div class="text-center"><span class="status-circle ${circleClass(
            item.Indicator
          )}" title="${item.Indicator}"></span></div></td>
      ...
      item.RowNum
    }, loadDataTab3)"><i class="bi bi-trash"></i></button></td>
...
function renderTableTab1(data) {
  ...
  data.forEach((item, idx) => {
    ...
    row.innerHTML = `
          ...
          <td><button class="btn btn-primary btn-sm" onclick='bukaModalTab1(${JSON.stringify(
            item
          )})'>Respon</button></td>
        `;
    body.appendChild(row);
...
function simpanKemaskiniTab1() {
  document.getElementById("responInput").value =
    quillResponInput.root.innerHTML;
      ...
      google.script.run
        .withSuccessHandler((link) => {
          ...
          google.script.run
            .withSuccessHandler(() => {
              bootstrap.Modal.getInstance(
                document.getElementById("kemaskiniModal1")
              ).hide();
              loadDataTab1();
              btn.disabled = false;
              btn.innerHTML = originalText;
...
function bukaModaTambahSyorBaharuPeneraju() {
  ...
  // Paparkan modal
  const tambahModal2 = new bootstrap.Modal(
    document.getElementById("tambahModal2")
  );
  tambahModal2.show();
}
...
function simpanKemaskiniTab2() {
  ...
  document.getElementById("catatanInput").value =
    quillCatatanInput.root.innerHTML;
  ...
  google.script.run
    .withSuccessHandler(() => {
      bootstrap.Modal.getInstance(
        document.getElementById("kemaskiniModal2")
      ).hide();
      loadDataTab2();
      btn.disabled = false;
      btn.innerHTML = originalText;
...
function simpanKemaskiniTab3() {
  ...
  document.getElementById("catatanInput2").value =
    quillCatatanInput2.root.innerHTML;
  ...
  google.script.run
    .withSuccessHandler(() => {
      bootstrap.Modal.getInstance(
        document.getElementById("kemaskiniModal3")
      ).hide();
      loadDataTab3();
      btn.disabled = false;
      btn.innerHTML = originalText;
    ...
    .withSuccessHandler(function (user) {
      ...
      google.script.run
        .withSuccessHandler(function (data) {
          ...
          document.getElementById("totalSelesai").textContent =
            statusCount.Selesai;
          document.getElementById("totalTindakan").textContent =
        ...
        .withSuccessHandler(function (data) {
          ...
          if (data && data.length > 0) {
            data.forEach((item, idx) => {
              const row = document.createElement("tr");
              const badgeClass =
                item.DominantStatus === "Hijau"
                  ? "bg-success"
                  : item.DominantStatus === "Kuning"
              ...
              row.innerHTML = `
                      ...
                      <td><span class="fw-bold">${
                        item.SkorWajaran
                      }</span> <span class="badge ${badgeClass}">${
                item.DominantStatus
              }</span></td>
                    `;
              body.appendChild(row);
...
function renderBahagianBarStackedChart(data) {
  ...
  const ctxBahagian = document
    ...
    .getContext("2d");
  const ctxNegeri = document
    .getElementById("statusChartNegeri")
    .getContext("2d");
  const ctxPeneraju = document
    .getElementById("statusChartPeneraju")
    .getContext("2d");
...
function formatDateForInput(dateStr) {
  ...
  if (parts.length === 3) {
    ...
    if (d.length === 4)
      return `${d}-${m.padStart(2, "0")}-${y.padStart(2, "0")}`; // yyyy-mm-dd
  }
  return "";
}
...
document.getElementById("searchInput1").addEventListener("input", function () {
  ...
  const filtered = dataTab1.filter(
    (item) =>
...
document.getElementById("searchInput2").addEventListener("input", function () {
  const val = this.value.toLowerCase();
  const filtered = dataTab2.filter(
    (item) =>
      (item.Laporan || "").toLowerCase().includes(val) ||
      (item.Syor || "").toLowerCase().includes(val) ||
      (item.Indicator || "").toLowerCase().includes(val) ||
...
function bukaModalDaftarPengguna() {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("daftarPenggunaModal")
  );
  modal.show();
}
...
function bukaModalKemaskiniPengguna(item, rowIndex) {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("kemaskiniPenggunaModal")
  );
  modal.show();
}
...
window.onload = function () {
  ...
  document.getElementById(
    "liveDate"
  ).innerText = `${hari}, ${tarikh} ${bulan} ${tahun}`;

  document
    .getElementById("formKemaskiniPengguna")
    .addEventListener("submit", function (e) {
      ...
      const btnKemaskiniUser = document.querySelector(
        "#kemaskiniPenggunaModal .btn-success"
      );
  ...
  document
    .getElementById("formDaftarPengguna")
    .addEventListener("submit", function (e) {
      e.preventDefault();
...
const quillCatatanBaru = new Quill("#editorCatatanBaru", {
  theme: "snow",
...
document
  .getElementById("formTambahSyor2")
  .addEventListener("submit", function (e) {
    document.getElementById("syorBaru2").value = quillSyorBaru2.root.innerHTML;
  });
